 ...
 *
 * This implementation provides a minimal subset of the Redis client API
 * used in this project:
 * - `connect()` / `quit()` lifecycle
 * - `get(key)`, `set(key, value, mode?, ttl?)`, `del(...keys)`, `expire(key, seconds)`
 * - `hget(hash, field)`, `hset(hash, field, value)`
 * - compatibility helpers: `setex(key, seconds, value)` and `scanStream({match, count})`
 *
 * The goal is compatibility for the project's needs (not a full Redis
 * implementation). Methods return values resembling `node-redis` where
 * reasonable (e.g. `set` returns `'OK'`).
 */
...
export default class InMemoriaRedis {
    ...
    /**
     * Set a key. Accepts both Redis-style `set(key, value, 'EX', seconds)`
     * and local `set(key, value, undefined, seconds)` usages.
     */
    async set(key: string, value: string, mode?: string | null, ttlSeconds?: number): Promise<'OK' | null> {
        this.store.set(key, value);
        if ((mode === 'EX' || !!ttlSeconds) && ttlSeconds && ttlSeconds > 0) {
    ...
    /**
     * Compatibility: simple scanStream implementation that emits 'data' with
     * arrays of matching keys and then 'end'. Options supports `{match, count}`.
     */
