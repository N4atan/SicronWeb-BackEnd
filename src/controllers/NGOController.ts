...
export class NGOController {
    ...
    static async register(req: Request, res: Response): Promise<Response> {
        console.log(`[DEBUG] NGOController.register called. Body:`, req.body);
        const {
            name,
            trade_name,
            cnpj,
        ...
        console.log(`[DEBUG] NGO registered successfully: ${created.uuid}`);
        return res.status(201).location(`/ngo/${created.uuid}`).send();
    }

    static async query(req: Request, res: Response): Promise<Response> {
        console.log(`[DEBUG] NGOController.query called. Query:`, req.query);
        const filters: any = {};
        ...
        const list = await NGOController.ngoRepository.findAll({ where: filters });
        console.log(`[DEBUG] NGOController.query found ${list.length} NGOs.`);
        return res.status(200).json({ ngos: list });
    }

    static async update(req: Request, res: Response): Promise<Response> {
        const ngo = req.ngo!;
        console.log(`[DEBUG] NGOController.update called. UUID: ${ngo.uuid}, Body:`, req.body);
        ...
        console.log(`[DEBUG] NGO updated successfully.`);
        return res.status(204).send();
    }

    static async delete(req: Request, res: Response): Promise<Response> {
        const ngo = req.ngo!;
        console.log(`[DEBUG] NGOController.delete called. UUID: ${ngo.uuid}`);

        if (ngo.manager_uuid !== req.user!.uuid && req.user!.role !== "admin")
            return res.status(403).json({ message: "Permissão negada!" });

        await NGOController.ngoRepository['repository'].remove(ngo);
        console.log(`[DEBUG] NGO deleted successfully.`);
        return res.status(204).send();
    }
}